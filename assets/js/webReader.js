document.addEventListener("DOMContentLoaded", () => {
  ...
  const updatePagedLayout = () => {
    ...
    if (!isPaged) {
      ...
      webReaderContent.style.paddingBottom = "";
    ...
    // Usar visualViewport SIEMPRE que esté disponible (más confiable que window.innerHeight)
    const vp = window.visualViewport;
    // Añadir buffer de 20px extra para safe-area del navegador
    const viewH = vp ? vp.height : window.innerHeight;
    const safeAreaBuffer = 20; // Buffer extra para menús del navegador

    // Altura de las barras propias del lector
    const headerH = webReaderHeader.offsetHeight || 60;
    const footerH = webReaderFooter.offsetHeight || 60;

    // Exponer variables CSS
    ...
    // Calcular altura del contenido CON BUFFER EXTRA
    const contentH = viewH - headerH - footerH - safeAreaBuffer;
    webReaderContent.style.height = `${contentH}px`;
    webReaderContent.style.marginTop = `${headerH}px`;
    webReaderContent.style.paddingBottom = `${safeAreaBuffer}px`; // Buffer abajo

    // Exponer altura del contenido para imágenes
    document.documentElement.style.setProperty(
      "--reader-content-h",
      `${contentH}px`,
    );
    ...
    // console.log('Paged Layout:', { viewH, headerH, footerH, contentH, vpHeight: vp?.height });
  };

  // Escuchar cambios del viewport (resize del visualViewport = chrome del nav)
  if (window.visualViewport) {
    // Escuchar resize (cuando el chrome del navegador entra/sale)
    window.visualViewport.addEventListener("resize", updatePagedLayout, {
      passive: true,
    });
    // Escuchar scroll (algunos navegadores disparan cambios de altura al hacer scroll)
    window.visualViewport.addEventListener("scroll", updatePagedLayout, {
      passive: true,
    });
  }
  ...
  window.addEventListener(
    "orientationchange",
    () => {
      setTimeout(updatePagedLayout, 300); // Delay para que el navegador termine de ajustar
    },
    { passive: true },
  );
  ...
  const loadSettings = () => {
    ...
    const savedLineHeight =
      localStorage.getItem("kasniaReaderLineHeight") || "1.6";
    const savedFont =
      localStorage.getItem("kasniaReaderFont") || "system-ui, sans-serif";
  ...
  webReaderContainer.addEventListener("click", (e) => {
    ...
    if (
      ...
      e.target.closest("a") ||
      e.target.closest(".webReaderHeader") ||
      e.target.closest(".webReaderSettings") ||
      e.target.closest(".footnoteToast")
    ) {
      return;
    }
      ...
      link.addEventListener("click", (e) => {
        ...
        if (href.startsWith("#")) {
          ...
          if (targetElement) {
            ...
            console.warn(
              "Footnote reference not found in current DOM: ",
              targetId,
            );
          }
        }
  ...
  const updateActiveToc = () => {
    const chunks = document.querySelectorAll(
      ".readerChunk[data-chapter-index]",
    );
    const isPaged = webReaderContent.classList.contains("pagedMode");
    let activeIndex = -1;
  ...
  webReaderContent.addEventListener(
    "scroll",
    () => {
      if (webReaderContent.classList.contains("pagedMode")) {
        if (typeof checkBoundaries === "function") checkBoundaries();
  ...
  const createPictureHTML = (pathsArr, altText = "") => {
    ...
    if (Array.isArray(pathsArr)) {
      pathsArr.forEach((p) => {
        if (p.endsWith(".avif"))
          html += `<source srcset="${basePath}${p}" type="image/avif">`;
        if (p.endsWith(".webp"))
          html += `<source srcset="${basePath}${p}" type="image/webp">`;
      });
      const fallback =
        pathsArr.find((p) => p.endsWith(".jpg") || p.endsWith(".png")) ||
        pathsArr[0];
  ...
  const fetchChapterNodes = async (fileIndex) => {
    ...
    try {
      ...
      if (fileObj.type === "cover" || fileObj.type === "image") {
        ...
        tempChunk.className = "readerChunk"; // Solo readerChunk, sin 'dimg grande' que puede causar conflictos
        tempChunk.setAttribute("data-chapter-index", fileIndex);
        tempChunk.setAttribute("data-type", "image-only"); // Marker para debugging
        tempChunk.innerHTML = createPictureHTML(fileObj.images, fileObj.title);
        nodes.push(tempChunk);
        return nodes; // RETURN INMEDIATO - nada más se añade
      }
      ...
      if (fileObj.type === "html") {
        ...
        if (
          childrenSource.length === 1 &&
          childrenSource[0].tagName === "SECTION"
        ) {
  ...
  const loadPreviousChapter = async () => {
    ...
    if (nodes.length > 0) {
      ...
      nodes
        .reverse()
        .forEach((node) => topBoundary.insertAdjacentElement("afterend", node));

      // Compensar el scroll por los nodos inyectados arriba/izquierda
      if (anchor) {
        if (isPaged) {
          const scrollDiff =
            anchor.getBoundingClientRect().left - offsetLeftBefore;
          webReaderContent.scrollBy({ left: scrollDiff, behavior: "instant" });
        } else {
          const scrollDiff =
            anchor.getBoundingClientRect().top - offsetTopBefore;
  ...
  const loadNextChapter = async () => {
    if (isFetchingChapter || currentEndFileIndex >= readingQueue.length - 1)
      return;
    isFetchingChapter = true;
    currentEndFileIndex++;
    ...
    if (currentEndFileIndex >= readingQueue.length - 1) {
      ...
      fin.innerHTML =
        "<b>Fin del volumen.</b><br>Gracias por leer Kasnia Project.";
  ...
  const checkBoundaries = () => {
    ...
    if (isPaged) {
      ...
      if (scrollL <= 300 && currentStartFileIndex > 0) {
        ...
        scrollL + clientW >= scrollW - 300 &&
        currentEndFileIndex < readingQueue.length - 1
      ) {
        loadNextChapter();
      }
    } else {
  ...
  loadIndexJson().then((success) => {
    if (success) {
      ...
      if (spinner)
        spinner.textContent = "Error: Faltan archivos de datos (index.json)";
    }
  });
});
